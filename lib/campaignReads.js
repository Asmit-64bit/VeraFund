...
const FACTORY_ADDRESSES = Array.from(new Set([FACTORY_ADDRESS, ...LEGACY_FACTORY_ADDRESSES]));
...
async function loadCampaignSnapshot(address, account, factoryAddress = FACTORY_ADDRESS) {
  ...
  let fundingUnlockModel = "legacy_goal_gated";

  try {
    // Newer campaign deployments expose bootstrapReleased().
    // If the probe succeeds, the UI should use threshold-based unlock messaging.
    bootstrapReleased = await readAny(`Bootstrap state ${address.slice(0, 8)} fetch`, async (provider) => {
      const contract = new ethers.Contract(address, CAMPAIGN_ABI, provider);
